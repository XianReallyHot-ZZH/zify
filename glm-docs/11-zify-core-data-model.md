...
| `tool_call_log` | 工具调用日志（大表，P2 归 tool 模块——执行点即记录点；对话/工作流都经 `ToolFacade` 触发） |
...
engine 是纯编排（ReAct 循环），**不持有任何表**（P2 A2 决策：engine 不碰 DB）。`tool_call_log` 原归 engine，P2 改归 tool 模块（见上）。
...
message(TOOL)   1:1   tool_call_log            一条 TOOL 消息对应一条工具调用日志（经 metadata.toolCallId 关联，P2 A2）
```
...
- `tool_call_log` 是统一工具系统的运行日志，**P2 归 tool 模块**（执行点即记录点）；触发方可是对话引擎（engine 经 `ToolFacade`）或工作流（workflow 经 `ToolFacade`），但执行与记录都在 tool 模块。
