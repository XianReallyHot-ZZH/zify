...
P1 **不新增** model 模块的表：流式 Chat 网关是纯代码增量（`infrastructure/client/` + `ModelFacade.chatStream`），复用已有的 `model_provider` / `model` 表读取供应商配置与解密 API Key。唯一对 `model` 表的结构改动是新增 `context_window` 列（见 §3.4），用于上下文预算计算。
...
CREATE TABLE `conversation` (
    ...
    `summary_text`               MEDIUMTEXT NULL COMMENT '历史摘要正文，覆盖到 summary_covered_message_id；NULL 表示尚无摘要',
    `summary_covered_message_id` CHAR(36)   NULL COMMENT '摘要已覆盖到的最后一条消息 ID；与 summary_text 同生共灭',
...
| `summary_text` | 否 | 历史摘要正文，覆盖 `[首条 .. summary_covered_message_id]` 的压缩。NULL 表示尚无摘要（新会话或未触发压缩）。见 `02-functional-spec.md` §5.5 |
| `summary_covered_message_id` | 否 | 摘要已覆盖到的最后一条消息 ID。「活窗口」= 此 ID 之后的消息（含本轮 user 输入）。与 `summary_text` 同生共灭（同时为 NULL 或同时非空） |
...
### 3.4 `model` 表增量 — `context_window`（P1 新增列）

P1 给 P0 已建的 `model` 表新增一列，供上下文预算计算（见 `02-functional-spec.md` §5.5）：
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `context_window` | 否 | 模型上下文窗口（如 128000）。为 NULL 时 engine 按 `zify.chat.context.default-window` 兜底，**不强制改造 P0 模型管理 UI** |

用独立列而非 `default_params` JSON：它是预算计算的一等属性，独立列更清晰、可后续查询。
...
V4__chat__create_conversation_message.sql  # 建 conversation（含 summary 列）、message 表
V5__model__add_context_window.sql          # model 表新增 context_window 列
```
...
| `conversation.summary_text` / `summary_covered_message_id` | 无 | 新增 | 支撑摘要压缩（P1 上下文管理，见 02 §5.5） |
| `model.context_window` | 无 | 新增列 | 上下文预算计算（P1，见 §3.4） |
